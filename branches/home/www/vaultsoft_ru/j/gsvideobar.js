...
GSvideoBar.prototype.buildSuperStructure = function() {
  ...
  if (this.externalMaster == null) {
    if ( this.playerRoot == GSvideoBar.PLAYER_ROOT_FLOATING ) {
      ...
      this.floatingPlayerBox.onScroll = function() {//don't context here

        var playerBounds = GSvideoBar.nodeBounds( self.playerRoot );
        var y = GSvideoBar.GetIEScrollTop( window ) + (GSvideoBar.getClientHeight() - playerBounds.height) /2;
        if ( "undefined" == typeof jQuery ) {
          self.playerRoot.style.top           = y + "px";
          self.floatingPlayerBox.style.top    = y - 10 + "px";
        } else {
          jQuery( self.playerRoot ).stop( true ).animate( { top: y }, 200 );
          jQuery( self.floatingPlayerBox ).stop( true ).animate( { top: y - 10 }, 200 );
        }
      }
    ...
    var f = this.floatingPlayerBox;
    
    if ( "undefined" != typeof jQuery ) {
      jQuery( window ).scroll( f.onScroll );
    }
  }
  this.player = null;
...
GSvideoBar.prototype.processResults = function(results) {
  if ( results && results.length > 0) {
    ...
    for (var i = 0; i < results.length && i < GSvideoBar.MAX_VIDEOS; i++) {
...
GSvideoBar.prototype.playVideo = function(result) {
  ...
  if ( result.playUrl && result.playUrl != "" ) {
    ...
    this.player.setAttribute( "data", result.playUrl );
        
    /*get a chance for a new tools*/
    var html5video = document.createElement( "video" );
    html5video.setAttribute( "src", result.playUrl);
    html5video.setAttribute( "controls", "controls" );
    html5video.setAttribute( "autoplay", "autoplay" );
    html5video.appendChild( GvideoSearch.createPlayer( result, this.CL_PLAYER ) );
    
    //this.player.appendChild( html5video );
    this.playerInnerBox.appendChild( this.player );
    //console.log( this.playerInnerBox );
    ...
    if (this.floatingPlayerBox) {
      ...
      if (this.verticalMode) {
        ...
        //y = bounds.y + (bounds.height - playerBounds.height) / 2 ;
        y = GSvideoBar.GetIEScrollTop( window ) + (GSvideoBar.getClientHeight() - playerBounds.height) /2;
        //console.log( "playerBounds.height", playerBounds.height, bounds, y  );
        //y = y - GSvideoBar.nodeBounds(this.brandingCell).height / 2;
        //console.log( y );
      ...
      if ( "undefined" == typeof jQuery ) {
        ...
        jQuery( this.playerRoot ).stop( true ).animate( {
          top: y,
          left: x
        }, 200 );
        jQuery( this.floatingPlayerBox ).stop( true ).animate( {
          top: y - 10,
          left: x - 10,
...
GSvideoBar.prototype.stopVideo = function() {
  ...
  if ( "undefined" != typeof jQuery ) {
    jQuery( this.playerBox ).stop( true ).hide();
    jQuery( this.floatingPlayerBox ).stop( true ).hide();
  }
}
...
GSvideoBar.nodeBounds = function(obj) {
  ...
  if (obj.getBoundingClientRect) {
    ...
    //console.log( obj, result.y, window.pageYOffset );
    return result;
  }
...
GSvideoBar.getClientWidth = function () {
  return document.compatMode=='CSS1Compat' && !window.opera?document.documentElement.clientWidth:document.body.clientWidth;
} 
 
GSvideoBar.getClientHeight = function () {
